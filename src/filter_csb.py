...
def get_result(filters): 
    file_path = Path(filters["path"])
    files = get_all_files(filters["path"], tree.root)
    ...
    tree.print_tree(files)
    return files

def get_all_files(path, parent): #this creates a binary tree along the way
    files = []
    for item in os.listdir(path):
        ...
        futureParent = tree.add_node(parent, item, item_path) #the child becomes the parent of the next item
        # recursion logic
        if os.path.isfile(item_path): files.append(item_path)
        else: files.extend(get_all_files(item_path, futureParent))
    return files
